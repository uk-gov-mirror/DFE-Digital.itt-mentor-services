...
RSpec.describe Claims::Providers::ApproveRejectSamplingClaimWizard::MentorTrainingStep, type: :model do
  ...
  describe "#max_hours" do
    it "returns the mentor training's hours completed" do
      expect(step.max_hours).to eq(10)
    end

    context "when there is no matching mentor training" do
      let(:attributes) { { mentor_id: create(:claims_mentor).id } }
